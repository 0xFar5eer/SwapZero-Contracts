...
contract SwapZero is swzERC1155, ReentrancyGuard {
  ...
  constructor() {
    ...
    listOfPools.push(
      Pool({ tokenInPool: IERC20(NATIVE_TOKEN), swzTokenBalance: 0 })
    );
  }
  ...
  function createPool(IERC20 _tokenAddr) public returns (uint256) {
    ...
    listOfPools.push(Pool({ tokenInPool: _tokenAddr, swzTokenBalance: 0 }));
    ...
    returns (uint256)
  {
    ...
    if (totalSupplyOfLiquidity == 0) {
      ...
      amountOfLiquidityToMint =
        _sqrt(_amountTokensIn * _amountSwzTokensIn) -
        MINIMUM_LIQUIDITY;
    } else {
      ...
      uint256 optimalAmountTokensIn = (_amountSwzTokensIn *
        poolTokenBalanceBefore) / pool.swzTokenBalance;
      if (optimalAmountTokensIn <= _amountTokensIn) {
        ...
        uint256 optimalAmountSwzTokensIn = (_amountTokensIn *
          pool.swzTokenBalance) / poolTokenBalanceBefore;

        require(
          optimalAmountSwzTokensIn <= _amountSwzTokensIn,
          "Insufficient _amountTokensIn & _amountSwzTokensIn provided"
        );
        require(
          optimalAmountSwzTokensIn >= _minAmountSwzTokensIn,
          "Insufficient _amountSwzTokensIn provided"
        );
    ...
    returns (uint256, uint256)
  {
    ...
    uint256 amountSwzTokensOut = (pool.swzTokenBalance *
      _amountOfLiquidityToRemove) / totalSupplyOfLiquidity;
    ...
    require(
      amountSwzTokensOut >= _minAmountSwzTokensOut,
      "AmountSwzTokensOut is less than minimum"
    );
    ...
    returns (uint256)
  {
    ...
    if (_tokenOut == swzToken) {
      ...
      amountSwzTokensOut = _getOutput(reservesIn, reservesOut, _amountTokensIn);

      // checking slippage
      require(
        amountSwzTokensOut >= _minAmountTokensOut,
        "Amount of tokens out is less than minimum required"
      );
    ...
    (reservesOut, reservesIn) = getPoolBalances(_tokenOut);
    amountTokensOut = _getOutput(reservesIn, reservesOut, amountSwzTokensOut);
    ...
    returns (uint256)
  {
    ...
    if (_tokenIn == swzToken) {
      ...
      amountSwzTokensIn = _getInput(reservesIn, reservesOut, _amountTokensOut);

      // checking slippage
      require(
        amountSwzTokensIn <= _maxAmountTokensIn,
    ...
    (reservesIn, reservesOut) = getPoolBalances(_tokenIn);
    amountTokensIn = _getInput(reservesIn, reservesOut, amountSwzTokensIn);
  ...
  function _swap(
    ...
    uint256 tokenBalanceAfter = _tokenBalanceBefore +
      _amountTokensIn -
      _amountTokensOut;
    uint256 swzTokenBalanceAfter = pool.swzTokenBalance +
      _amountSwzTokensIn -
      _amountSwzTokensOut;
    ...
    uint256 kValueAfter = (tokenBalanceAfter -
      ...
      (swzTokenBalanceAfter -
        (_amountSwzTokensIn * TRADE_FEE_NOMINATOR) /
        TRADE_FEE_DENOMINATOR);
    ...
    emit Sync(_tokenAddr, tokenBalanceAfter, swzTokenBalanceAfter);
  }
  ...
  function test1_CreateTokensAndPools() public {
    testTokenIn = ITestErc20Token(
      address(new TestErc20Token("TokenIn", "USDC"))
    );
    testTokenOut = ITestErc20Token(
  ...
  function test3_AddLiquidity_RemoveLiqudity() public {
    (uint256 reservesTokenIn, uint256 reservesSwzTokenOut) = getPoolBalances(
      testTokenIn
    );

    uint256 initialAmountOfTokenIn = 1e3 * 1e18;
    uint256 initialAmountOfSwzToken = (initialAmountOfTokenIn *
      reservesSwzTokenOut) / reservesTokenIn;
    ...
    require(
      amountOfSwzTokensOut < initialAmountOfSwzToken,
      "amountOfSwzTokensOut <= initialAmountOfSwzToken"
    );
  }
  ...
  function test4_AddLiquidity_SwapAndBack_RemoveLiqudity() public {
    ...
    uint256 addLiquiditySwz = (addLiquidityTokens * reservesSwzTokenOut) /
      reservesTokenIn;
    ...
    require(
      amountOfSwzTokensOut < addLiquiditySwz,
      "amountOfSwzTokensOut < addLiquiditySwz"
    );
  }
